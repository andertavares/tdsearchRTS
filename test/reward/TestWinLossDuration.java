...
class TestWinLossDuration {
	...
	void testReward() throws Exception {
		...
		WinLossDuration model = new WinLossDuration(3000);
	...
	void testGameOverReward() throws Exception {
		...
		WinLossDuration model = new WinLossDuration(3000);
		// at 1/3 duration, p0 gets 2/3 and p1 gets -2/3 on p0wins
		setGameTime(p0Wins, 1000); 
		assertEquals(2.0 / 3, model.reward(p0Wins, 0), 1E-10);
		assertEquals(-2.0 / 3, model.reward(p0Wins, 1), 1E-10);
		// signals flipped for p1wins
		setGameTime(p1Wins, 1000); 
		assertEquals(-2.0 / 3, model.reward(p1Wins, 0), 1E-10);
		assertEquals(2.0 / 3, model.reward(p1Wins, 1), 1E-10);
		
		
		// at half duration, p0 gets 1/2 and p1 gets -1/2 on p0wins
		setGameTime(p0Wins, 1500); 
		assertEquals(0.5, model.reward(p0Wins, 0), 1E-10);
		assertEquals(-0.5, model.reward(p0Wins, 1), 1E-10);
		// flipping signals for p1wins
		setGameTime(p1Wins, 1500); 
		assertEquals(-0.5, model.reward(p1Wins, 0), 1E-10);
		assertEquals(0.5, model.reward(p1Wins, 1), 1E-10);
		
		
		// p0 won at the but-last frame
		setGameTime(p0Wins, 2999); 
		assertEquals(1/3000.0, model.reward(p0Wins, 0), 1E-10);
		assertEquals(-1/3000.0, model.reward(p0Wins, 1), 1E-10);
		// p1 won at the but-last frame
		setGameTime(p1Wins, 2999); 
		assertEquals(-1/3000.0, model.reward(p1Wins, 0), 1E-10);
		assertEquals(1/3000.0, model.reward(p1Wins, 1), 1E-10);
		
		//at max duration, both get 0
		setGameTime(p0Wins, 3000);
		assertEquals(0, model.reward(p0Wins, 0), 1E-10);
		assertEquals(0, model.reward(p0Wins, 1), 1E-10);
		
		setGameTime(p1Wins, 3000);
		assertEquals(0, model.reward(p1Wins, 0), 1E-10);
		assertEquals(0, model.reward(p1Wins, 1), 1E-10);
	}
