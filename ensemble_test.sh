...
# Runs test matches. (some) Parameters:
# -c, --config-input,  Input config path
# -d, --working_dir, Directory where results were (and will be) saved
...
# ./ensemble.sh -c config/selfplay-basesWorkers8x8.properties -t ai.abstraction.LightRush -d results/selfplay-basesWorkers8x8/ -i 0 -f 9
...
echo "Launching test ensemble matches..."

java -classpath $classpath -Djava.library.path=lib/ main.TestEnsemble "$@" 
