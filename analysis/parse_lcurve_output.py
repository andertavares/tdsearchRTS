...
def main(basedir, raw_output='raw.csv', avg_output='avg.csv', sep=',', test_opp='A3N', file_prefix='lcurve-vs-', search_timebudget=None):
    ...
    for rep_dir in tqdm(glob.glob(os.path.join(basedir, *['*'] * 7))):
        ...
        for test_pos in [0, 1]:
            ...
            exp_result_pattern = os.path.join(rep_dir, '%s%s_p%d_m*.csv' % (file_prefix, test_opp, test_pos))
            if search_timebudget is not None:
                exp_result_pattern = os.path.join(rep_dir, '%s%s_p%d_m*_b%d.csv' % (file_prefix, test_opp, test_pos, search_timebudget))
